...
def res_cards(res_list, pfx, show_email=False):
    ...
    for r in sorted(res_list, key=lambda x: x["date"], reverse=True):
        ...
        note      = ("Cancel anytime for a full refund." if not FA else "هر زمان لغو کنید — بازپرداخت کامل.") if not cancelled else ("Refunded." if not FA else "بازپرداخت شده.")

        # Build meta cells without nesting f-strings inside f-strings
        ...
        if not cancelled:
            ...
            if st.button(
                "Cancel & Refund" if not FA else "لغو و بازپرداخت",
                key=f"{pfx}_c_{r['id']}"
            ):
                cancel_res(r["id"])
                st.rerun()

...
# ─────────────────────────────────────────────
def hall_calendar(hall_key: str, days_ahead: int = 14, highlight_email: str = None):
    """
    Shows a heatmap-style calendar for ONE hall.
    X = upcoming dates, Y = hour slots (09:00–20:00).
    Cells:  FREE (cream) | BOOKED-other (wine) | BOOKED-mine (gold) | UNAVAILABLE (grey)
    """
    FA = is_fa()
    hall = HALLS[hall_key]
    ...
    today = date.today()
    date_range = [str(today + timedelta(days=i)) for i in range(days_ahead)]
    hours = list(range(9, 21))   # 9 → 20 inclusive

    # Collect all booked (hall, date, hour) -> res_info
    booked = {}   # (date, hour) -> {"name", "email", "slot_label", "price", "id"}
    for r in st.session_state.reservations:
    ...
    # z: 0=free, 1=booked-other, 2=booked-mine
    z_vals    = []
    hover     = []
    y_labels  = [f"{h:02d}:00" for h in hours]
    ...
    for h in hours:
        ...
        for d in date_range:
            ...
            if info is None:
                z_row.append(0)
                h_row.append(
                    f"<b>{'Free' if not FA else 'آزاد'}</b><br>"
                    f"{d} {'at' if not FA else 'ساعت'} {h:02d}:00"
                )
            elif info["mine"]:
                z_row.append(2)
                h_row.append(
                    f"<b>{'Your Booking' if not FA else 'رزرو شما'}</b><br>"
                    f"{d} {info['slot']}<br>"
                    f"{'Guest' if not FA else 'مهمان'}: {info['name']}<br>"
                    f"{info['price']}"
                )
            else:
                z_row.append(1)
                h_row.append(
                    f"<b>{'Booked' if not FA else 'رزرو شده'}</b><br>"
                    f"{d} {info['slot']}<br>"
                    f"{'Guest' if not FA else 'مهمان'}: {info['name']}<br>"
                    f"{info['price']}"
                )
    ...
    # Short date labels for x-axis  e.g. "Mon 02"
    x_labels = []
    for d in date_range:
        dt = datetime.strptime(d, "%Y-%m-%d")
        x_labels.append(dt.strftime("%a %d" if not FA else "%d"))

    colorscale = [
        [0.0,  "#f5eadf"],   # free — cream
        [0.5,  "#f5eadf"],
        [0.501, hall["color"]],  # booked other — hall colour
        [0.85,  hall["color"]],
        [0.851, GOLD],       # my booking — gold
        [1.0,   GOLD],
    ]
    ...
    # Overlay text labels inside cells
    annotations = []
    for yi, h in enumerate(hours):
        for xi, d in enumerate(date_range):
            info = booked.get((d, h))
            if info:
                short = (info["name"].split()[0] if info["name"] else "")[:8]
                annotations.append(dict(
                    x=x_labels[xi], y=f"{h:02d}:00",
                    text=f"<b>{short}</b>",
    ...
    fig.update_layout(
        title=dict(
            text=f"{hall['icon']} {hall_name} — {'Availability Calendar' if not FA else 'تقویم موجودی'}",
            font=dict(family="Playfair Display, serif", size=17, color=WINE),
            x=0.5,
        ),
        xaxis=dict(
            side="top",
            tickfont=dict(size=11, color="#5e4d46", family="DM Sans"),
            showgrid=False,
            fixedrange=True,
            title=dict(text=""),
        ),
    ...
    # Legend
    legend_html = f"""
    <div style="display:flex;gap:18px;align-items:center;margin-bottom:8px;font-size:12px;flex-wrap:wrap">
      <div style="display:flex;align-items:center;gap:6px">
        <div style="width:14px;height:14px;border-radius:3px;background:#f5eadf;border:1px solid #e7d8c6"></div>
        <span style="color:var(--muted)">{"Free" if not FA else "آزاد"}</span>
      </div>
      <div style="display:flex;align-items:center;gap:6px">
        <div style="width:14px;height:14px;border-radius:3px;background:{hall['color']}"></div>
        <span style="color:var(--muted)">{"Booked" if not FA else "رزرو شده"}</span>
      </div>
      {"" if not highlight_email else f'''
      <div style="display:flex;align-items:center;gap:6px">
...
def booking_form(prefix, by_email, by_name, is_owner=False):
    ...
    if submitted:
        ...
        if start_hr is None:
            ...
            new = {
                "id": gen_id(), "name": actual_name, "email": actual_email,
...
def res_cards(res_list, pfx, show_email=False):
    ...
    for r in sorted(res_list, key=lambda x: x["date"], reverse=True):
        ...
        badge = ("badge-cancelled" if cancelled else "badge-active")
        btxt  = ("✕ Cancelled" if not FA else "✕ لغو شده") if cancelled else ("✓ Confirmed" if not FA else "✓ تأیید شده")
        obadge = (f'<span class="badge-owner">👑 {"Owner" if not FA else "مدیر"}</span>'
                  if r.get("booked_by")=="owner" else "")
        email_col = (f'<div><div style="font-size:9px;font-weight:600;text-transform:uppercase;color:#9b7250">Email</div>'
                     f'<div style="margin-top:2px;font-size:11px;color:#4b3932;word-break:break-all">{r["email"]}</div></div>') if show_email else ""
        ...
        <div class="res-card {'cancelled' if cancelled else ''}">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap">
            <div>
              ...
              <span class="{badge}">{btxt}</span> {obadge}
            </div>
            <div class="res-price">{fmt(r['price'])}</div>
          </div>
          <div style="display:grid;grid-template-columns:repeat({cols},1fr);gap:8px;margin-top:10px;font-size:12px">
            <div><div style="font-size:9px;font-weight:600;text-transform:uppercase;color:#9b7250">{"Guest" if not FA else "مهمان"}</div>
              <div style="margin-top:2px;font-weight:500;color:#4b3932">{r["name"]}</div></div>
            {email_col}
            <div><div style="font-size:9px;font-weight:600;text-transform:uppercase;color:#9b7250">{"Date" if not FA else "تاریخ"}</div>
              <div style="margin-top:2px;font-weight:500;color:#4b3932">{r["date"]}</div></div>
            <div><div style="font-size:9px;font-weight:600;text-transform:uppercase;color:#9b7250">{"Slot" if not FA else "بازه"}</div>
              <div style="margin-top:2px;font-weight:500;color:#4b3932">{r["slot_label"]}</div></div>
            <div><div style="font-size:9px;font-weight:600;text-transform:uppercase;color:#9b7250">{"Dur." if not FA else "مدت"}</div>
              <div style="margin-top:2px;font-weight:500;color:#4b3932">{r["duration"]}h</div></div>
          </div>
          <div style="margin-top:10px;padding-top:8px;border-top:1px solid #eee0d3;font-size:11px;color:var(--muted)">
            {"Cancel anytime for a full refund." if not cancelled and not FA
             ...
             else ("Refunded." if not FA else "بازپرداخت شده.")}
          </div>
        </div>
        """, unsafe_allow_html=True)
        if not cancelled:
    ...
    with mid:
        ...
        if sub:
            ...
            if not rn.strip():
                ...
                st.session_state.users[ek] = {"name": rn.strip(), "email": ek, "pass_hash": hpw(rp1)}
...
# ──────────────────────────────
            ...
            <div class="panel-dark">
              ...
              <div style="font-size:12px;color:rgba(255,255,255,.72);line-height:1.7">
                ...
                <div style="display:flex;gap:8px"><span style="color:var(--gold);font-weight:700;flex-shrink:0">04</span>
                  <span>{"Cancel anytime for a full refund." if not FA else "هر زمان لغو کنید — بازپرداخت کامل."}</span></div>
              </div>
            </div>
            <div class="panel">
    ...
    with tab_cal:
        ...
        <p style="font-size:13px;color:var(--muted);margin-bottom:16px">
          {"Select a hall to see exactly which days and times are free or booked. "
           "Your own reservations are highlighted in gold."
           if not FA else
           "یک سالن را انتخاب کنید تا ببینید دقیقاً کدام روزها و ساعات آزاد یا رزرو شده است. "
           "رزروهای شما با رنگ طلایی مشخص شده است."}
        </p>
        """, unsafe_allow_html=True)
        ...
        # Pass user email so their bookings show gold
        hall_calendar(sel_hall, days_ahead=days, highlight_email=cu)
...
# ──────────────────────────────
    ...
    with tb2:
        FA = is_fa()
        st.markdown(f"""<div class="eyebrow">{"Full Reservation Calendar" if not FA else "تقویم کامل رزروها"}</div>
        <h2 style="font-family:'Playfair Display',serif;font-size:22px;margin-bottom:4px">
          {"All Studios · All Days" if not FA else "همه استودیوها · همه روزها"}</h2>
        <p style="font-size:13px;color:var(--muted);margin-bottom:16px">
          {"Select a hall — X axis shows days (above), Y axis shows hours. Hover for guest details."
           if not FA else "یک سالن انتخاب کنید — محور X روزها (بالا) و محور Y ساعات را نشان می‌دهد. نگه دارید تا جزئیات مهمان را ببینید."}
        </p>""", unsafe_allow_html=True)
        ...
        hall_calendar(o_hall, days_ahead=o_days, highlight_email=None)
    ...
    with tb4:
        ...
        with ac1:
            ...
            fig.update_layout(title=dict(text="Bookings by Guest" if not FA else "رزرو به تفکیک مهمان",
                                         font=dict(size=13,color=WINE),x=.5),
                              plot_bgcolor=CREAM,paper_bgcolor=CREAM,
                              xaxis=dict(showgrid=False),yaxis=dict(gridcolor="#e7d8c6"),
                              margin=dict(t=40,b=20,l=10,r=10),height=260,showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        ...
        with ac2:
            ...
            fig.update_layout(title=dict(text="Hall Usage" if not FA else "سهم سالن‌ها",
                                         font=dict(size=13,color=WINE),x=.5),
                              plot_bgcolor=CREAM,paper_bgcolor=CREAM,
        ...
        with ac3:
            ...
            fig = pgo.Figure(pgo.Scatter(
                x=dates,y=[dc[d] for d in dates],mode="lines+markers",
                line=dict(color=WINE,width=2.5),
                marker=dict(color=WINE,size=7,line=dict(color="#fffaf3",width=2)),
                fill="tozeroy",fillcolor="rgba(109,31,42,.08)",
                hovertemplate="<b>%{x}</b><br>Bookings: %{y}<extra></extra>",
            ))
            fig.update_layout(title=dict(text="Bookings Over Time" if not FA else "روند رزرو",
                                         font=dict(size=13,color=WINE),x=.5),
                              plot_bgcolor=CREAM,paper_bgcolor=CREAM,
                              xaxis=dict(showgrid=False),yaxis=dict(gridcolor="#e7d8c6"),

    ...
    with mid:
        ...
        if sub:
            ...
            if not rn.strip():
                ...
                st.session_state.users[ek] = {"name": rn.strip(), "email": ek, "pass_hash": hpw(rp1), "user_status":"active"}
...
# ──────────────────────────────
    ...
    with mid:
        ...
        if sub:
            ...
            elif u.get("user_status","active") == "suspended":
                st.error("Account suspended. Contact administrator." if not FA else "حساب کاربری مسدود شده است.")
            else:
...
# ──────────────────────────────
    ...
    with tb4:
        ...
        CHART_TICK = "#000000"   # dark ink — clearly readable on cream
    ...
    with tb6:
        ...
        search_user = st.text_input("Search users", key="user_search")
        users = st.session_state.users
        filtered = {e:u for e,u in users.items() if not search_user or search_user.lower() in e.lower() or search_user.lower() in u.get("name","").lower()}
        st.metric("Total Users", len(users))
        for email, user in filtered.items():
            bookings = [r for r in st.session_state.reservations if r["email"]==email]
            active = [r for r in bookings if r["status"]=="active"]
            spent = sum(r["price"] for r in bookings)
            with st.expander(f'{user.get("name","Unknown")} • {email}'):
                st.write("Status:", user.get("user_status","active"))
                st.write("Total bookings:", len(bookings))
                st.write("Active bookings:", len(active))
                st.write("Revenue:", fmt(spent))
                c1,c2,c3 = st.columns(3)
                with c1:
                    if st.button("Suspend", key=f"s_{email}"):
                        st.session_state.users[email]["user_status"]="suspended"
                        st.rerun()
                with c2:
                    if st.button("Activate", key=f"a_{email}"):
                        st.session_state.users[email]["user_status"]="active"
                        st.rerun()
                with c3:
                    if st.button("Delete User", key=f"d_{email}"):
                        st.session_state.reservations=[r for r in st.session_state.reservations if r["email"]!=email]

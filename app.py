...
# ─────────────────────────────────────────────
st.set_page_config(
    page_title=("پادمارت" if st.session_state.get("lang") == "fa" else "Padmart — Acting School"),
    page_icon="🎭",
    layout="wide",
...
# ─────────────────────────────────────────────
#  JALALI (SHAMSI) CALENDAR — pure-python, no deps
# ─────────────────────────────────────────────
_FA_WEEKDAYS = ["دوشنبه", "سه‌شنبه", "چهارشنبه", "پنجشنبه", "جمعه", "شنبه", "یکشنبه"]
_FA_WEEKDAYS_SHORT = ["دوش", "سه‌ش", "چهارش", "پنج‌ش", "جمعه", "شنبه", "یک‌ش"]
_FA_MONTHS = [
...
def gregorian_to_jalali(g_year, g_month, g_day):
    """Convert Gregorian (y,m,d) -> Jalali (y,m,d). Pure-python, no deps."""
...
def to_jalali_str(d, with_weekday=False, short=False):
    """d: date/datetime object or 'YYYY-MM-DD' string -> Shamsi display string."""
    if isinstance(d, str):
        d = datetime.strptime(d, "%Y-%m-%d").date()
    jy, jm, jd = gregorian_to_jalali(d.year, d.month, d.day)
    month_nm = _FA_MONTHS[jm - 1]
    out = to_fa_digits(jd) + " " + month_nm + " " + to_fa_digits(jy)
    if with_weekday:
        wd = _FA_WEEKDAYS_SHORT[d.weekday()] if short else _FA_WEEKDAYS[d.weekday()]
        out = wd + " " + out
    return out

def to_jalali_short(d):
    """Compact 'weekday day-month' label for calendar axis, e.g. 'شنبه ۱۲ تیر'."""
    if isinstance(d, str):
        d = datetime.strptime(d, "%Y-%m-%d").date()
    jy, jm, jd = gregorian_to_jalali(d.year, d.month, d.day)
    wd = _FA_WEEKDAYS_SHORT[d.weekday()]
    return wd + " " + to_fa_digits(jd) + " " + _FA_MONTHS[jm - 1][:3]

def display_date(d, with_weekday=False, short=False):
    """Locale-aware date display: Shamsi in FA mode, Gregorian otherwise."""
    if is_fa():
        return to_jalali_str(d, with_weekday=with_weekday, short=short)
    if isinstance(d, str):
        d = datetime.strptime(d, "%Y-%m-%d").date()
    return d.strftime("%a %d %b %Y") if with_weekday else d.strftime("%Y-%m-%d")
...
# ─────────────────────────────────────────────
#  RTL / RIGHT-ALIGN  (Persian mode only)
#  Streamlit containers don't inherit direction from <body>, so we force
#  RTL + right text-alignment across blocks, inputs, labels, tabs, cards.
#  This block is injected ONLY in FA mode, so English stays LTR/left.
# ─────────────────────────────────────────────
RTL_BLOCK = """
<style>
html, body,
...
/* All text content right-aligned */
...
/* Keep flex strips behaving (space-between rows shouldn't be forced right) */
.summary-strip, .res-card > div[style*="space-between"] {
  text-align: initial !important;
}

/* Inputs: caret and text start from the right */
.stTextInput input,
.stNumberInput input,
.stDateInput input,
.stTextArea textarea {
...
/* Latin tokens (reservation IDs, emails) read better left-to-right even in RTL */
.res-id {
...
st.markdown(RTL_BLOCK, unsafe_allow_html=True)
...
# Force-update the browser tab title — Streamlit's set_page_config only
# paints the title on first load; reruns don't reliably repaint <title>,
# and st.markdown's <script> runs inside a sandboxed iframe, so we must
# reach through to the parent document.
_tab_title = "پادمارت" if st.session_state.lang == "fa" else "Padmart — Acting School"
st.markdown(
    f"""<script>
    try {{ window.parent.document.title = {_tab_title!r}; }} catch (e) {{}}
    </script>""",
    unsafe_allow_html=True
)
...
def hall_calendar(
    ...
    for h in hours:
        z_row, h_row = [], []
        for d in date_range:
            d_disp = to_jalali_str(d) if FA else d
            info = booked.get((d, h))
            if info is None:
                z_row.append(0)
                h_row.append(
    ...
    for d in date_range:
        ...
        x_lbl.append(to_jalali_short(dt) if FA else dt.strftime("%a %d"))
    ...
    with st.form(f"cal_book_{cal_prefix}_{hall_key}"):
        ...
        cb_date = st.date_input(
            "Date" if not FA else "تاریخ (میلادی)",
            min_value=today_v,
        ...
        if FA:
            st.markdown(
                '<div style="font-size:13px;color:' + WINE + ';font-weight:600;margin:-8px 0 10px 2px">'
                '📅 ' + to_jalali_str(cb_date, with_weekday=True) +
                '</div>',
                unsafe_allow_html=True
            )
        cb_dur = st.slider(
...
def booking_form(prefix, by_email, by_name, is_owner=False):
    ...
    with st.form(f"book_{prefix}"):
        ...
        bdate = st.date_input("Date" if not FA else "تاریخ (میلادی)",
        ...
        if FA:
            st.markdown(
                '<div style="font-size:13px;color:' + WINE + ';font-weight:600;margin:-8px 0 10px 2px">'
                '📅 ' + to_jalali_str(bdate, with_weekday=True) +
                '</div>',
                unsafe_allow_html=True
            )
...
def res_cards(res_list, pfx, show_email=False):
    ...
    for r in sorted(res_list, key=lambda x: x["date"], reverse=True):
        ...
        meta_date  = _cell("Date"  if not FA else "تاریخ",  display_date(r["date"]))
...
if st.session_state.page == "gateway":
    ...
    <div class="hero" style="text-align:center;padding:52px 32px">
      ...
      <h1>{"پادمارت" if FA else "Padmart"}</h1>
...
<div style="border-top:1px solid var(--line);padding:18px 0;text-align:center;margin-top:36px">
  <span style="font-family:'Playfair Display',serif;font-size:15px;color:var(--wine)">{"پادمارت" if FA else "Padmart"}</span>
